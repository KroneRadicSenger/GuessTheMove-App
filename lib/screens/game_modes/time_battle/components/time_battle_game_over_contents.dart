...
class TimeBattleGameOverContents extends StatelessWidget {
  ...
  TimeBattleGameOverContents(
      {this.titleText = 'Deine Zeit ist vorbei',
  ...
  Widget _buildHighscoreBoxContents(final BuildContext context) {
    ...
    return FutureBuilder<TimeBattleGamePlayed?>(
      future: TimeBattleGamesPlayedDao().getHighscoreGamePlayedByAnalyzedGamesBundleAndInitialTimeInSeconds(analyzedGamesOriginBundle, initialTimeInSeconds),
  ...
  List<Widget> _buildGamesPlayedBoxes(final BuildContext context) {
    if (analyzedGamesPlayed != null) {
      return analyzedGamesPlayed!.map((game) {
        return _buildGamePlayedBox(context, analyzedGamesPlayed!.indexOf(game), game.whitePlayer, game.blackPlayer, [
          GameTitleText(userSettingsState, GameModeEnum.timeBattle, '${game.whitePlayer.getFirstAndLastName()} vs ${game.blackPlayer.getFirstAndLastName()}'),
    ...
    return gamesPlayedInfo!.map((gamePlayedInfo) {
      return _buildGamePlayedBox(context, gamesPlayedInfo!.indexOf(gamePlayedInfo), gamePlayedInfo.whitePlayer, gamePlayedInfo.blackPlayer, [
        GameTitleText(userSettingsState, GameModeEnum.timeBattle, '${gamePlayedInfo.whitePlayer.getFirstAndLastName()} vs ${gamePlayedInfo.blackPlayer.getFirstAndLastName()}'),
              ...
              Expanded(
                child: TextButton.icon(
                  ...
                  style: TextButton.styleFrom(
                    backgroundColor: appTheme(context, userSettingsState.userSettings.themeMode).gameModeThemes[GameModeEnum.timeBattle]!.accentColor,
  ...
  void _onPressShowGameInfo(final BuildContext context) {
    ...
    if (initialTimeInSeconds > 60 && initialTimeInSeconds % 60 == 0) {
      startTimeDisplayName = (initialTimeInSeconds ~/ 60).toString() + 'min';
    } else {
      startTimeDisplayName = initialTimeInSeconds.toString() + 's';
    }
