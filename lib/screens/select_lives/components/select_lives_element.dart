...
class SelectLivesElement extends StatelessWidget {
  final Player grandmaster;
  final AnalyzedGamesBundle analyzedGameOriginBundle;
  final int amountLives;
  final Function(int, Future<List<AnalyzedGame>>) onSelectLives;

  const SelectLivesElement({Key? key, required this.grandmaster, required this.analyzedGameOriginBundle, required this.amountLives, required this.onSelectLives}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<UserSettingsBloc, UserSettingsState>(
      builder: (context, state) => FutureBuilder<SurvivalGamePlayed?>(
        future: SurvivalGamesPlayedDao().getHighscoreGamePlayedByAnalyzedGamesBundleAndAmountLives(analyzedGameOriginBundle, amountLives),
                ...
                children: [
                  Expanded(
                    ...
                    child: Column(
                      ...
                      children: [
                        GameTitleText(state, GameModeEnum.survivalMode, amountLives.toString() + ' Leben'),
  ...
  List<Widget> _buildHighscoreInfo(final BuildContext context, final UserSettingsState userSettingsState, final SurvivalGamePlayed? highscoreGamePlayed) {
  ...
  void _onTap(final BuildContext context) async {
    ...
    onSelectLives(amountLives, gamesInBundleFuture);
  }

  void _onTapInfoIcon(final BuildContext context, final UserSettingsState userSettingsState, final SurvivalGamePlayed highscoreGamePlayed) {
