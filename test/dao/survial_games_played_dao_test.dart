...
const testDatabasePath = 'test/dao/db/survivalgamesplayeddaotest.db';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  group('SurvivalGamesPlayedDao test', () {
    ...
    final amountLives1 = 5;
    final amountLives2 = 5;
    final amountLives3 = 10;
    ...
    debugPrint('Running SurvivalGamesPlayedDao tests..');

    test('no data is retrieved before inserting data', () async {
      var gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives1);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives2);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives3);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByGrandmaster(grandmaster);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByPlayedDay(testDateTime1);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByPlayedDay(testDateTime2);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByPlayedDay(testDateTime3);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByPlayedDateInRange(testDateTime1, testDateTime3);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed, isEmpty);

      final newestGamePlayed = await SurvivalGamesPlayedDao(database: db).getNewest();
      expect(newestGamePlayed, isNull);

      var highscoreGamePlayed = await SurvivalGamesPlayedDao(database: db).getHighscoreGamePlayedByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives1);
      expect(highscoreGamePlayed, isNull);

      highscoreGamePlayed = await SurvivalGamesPlayedDao(database: db).getHighscoreGamePlayedByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives2);
      expect(highscoreGamePlayed, isNull);

      highscoreGamePlayed = await SurvivalGamesPlayedDao(database: db).getHighscoreGamePlayedByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives3);
      expect(highscoreGamePlayed, isNull);
    });

    test('insert and retrieve games played', () async {
      final gamePlayed1 = SurvivalGamePlayed(
        grandmaster: grandmaster,
        amountLives: amountLives1,
      ...
      final gamePlayed2 = SurvivalGamePlayed(
        grandmaster: grandmaster,
        amountLives: amountLives2,
        analyzedGameOriginBundle: analyzedGamesBundle,
      ...
      final gamePlayed3 = SurvivalGamePlayed(
        grandmaster: grandmaster,
        amountLives: amountLives3,
        analyzedGameOriginBundle: analyzedGamesBundle,
      ...
      await SurvivalGamesPlayedDao(database: db).insert(gamePlayed1);
      ...
      var newestGamePlayed = await SurvivalGamesPlayedDao(database: db).getNewest();
      ...
      await SurvivalGamesPlayedDao(database: db).insert(gamePlayed2);

      // Test retrieving first and second game played

      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives1);
      expect(gamesPlayed, isNotNull);
      ...
      newestGamePlayed = await SurvivalGamesPlayedDao(database: db).getNewest();
      expect(newestGamePlayed, isNotNull);
      expect(newestGamePlayed, gamePlayed2);

      highscoreGamePlayed = await SurvivalGamesPlayedDao(database: db).getHighscoreGamePlayedByAnalyzedGamesBundleAndAmountLives(analyzedGamesBundle, amountLives1);
      expect(highscoreGamePlayed, isNotNull);
      ...
      await SurvivalGamesPlayedDao(database: db).insert(gamePlayed3);
      ...
      gamesPlayed = await SurvivalGamesPlayedDao(database: db).getByPlayedDateInRange(testDateTime1, testDateTime2);
      expect(gamesPlayed, isNotNull);
      expect(gamesPlayed.length, 2);
      ...
      await SurvivalGamesPlayedDao(database: db).deleteAll();
